...
def _apply_request_timing_metadata(seq, metadata):
    """Apply the API receive anchor to the scheduler-visible Sequence.

    ``Sequence`` is constructed only after text tokenization / multimodal
    preprocessing.  Overwriting its constructor timestamp here ensures those
    costs are included in both observed TTFT and scheduler slack.
    """
    if not isinstance(metadata, dict):
        return seq

    arrival = metadata.get("_arrival_time")
    if arrival is not None:
        arrival = float(arrival)
        if not math.isfinite(arrival) or arrival <= 0:
            raise ValueError("invalid request arrival_time")
        seq.arrival_time = arrival

    deadline = metadata.get("_ttft_deadline")
    if deadline is not None:
        deadline = float(deadline)
        if not math.isfinite(deadline):
            raise ValueError("invalid TTFT deadline")
        seq.ttft_deadline = deadline

    tbt_slo_s = metadata.get("_tbt_slo_s")
    if tbt_slo_s is not None:
        tbt_slo_s = float(tbt_slo_s)
        if not math.isfinite(tbt_slo_s) or tbt_slo_s <= 0:
            raise ValueError("invalid TBT SLO")
        seq.tbt_slo_s = tbt_slo_s
...
class TokenizerProcessor(mp.Process):
    ...
    def run(self):
        ...
        while True:
            ...
            if len(item) == 4:
                request_id, prompt, sampling_params, multimodal = item
            else:
            ...
            if isinstance(prompt, str):
                ...
                token_ids = prompt
            self.output_queue.put((request_id, token_ids, sampling_params, multimodal))
...
class RequestTracker:
    ...
    def add_request(self, request_id: str, prompt: Union[str, List[int]], sampling_params: SamplingParams,
                    multimodal: dict = None) -> AsyncStream:
        """
        multimodal (optional): {
            'images': torch.Tensor,
        ...
        self._new_in.put_nowait((request_id, prompt, sampling_params, multimodal))
        return stream
    ...
    def get_new_and_finished(self) -> Tuple[List[Tuple[AsyncStream, str, Union[str, List[int]], SamplingParams]], Set[str]]:
        ...
        while not self._new_out.empty():
            item = self._new_out.get_nowait()
            
            if len(item) == 4:
                rid, prompt, sp, multimodal = item
            else:
                rid, prompt, sp = item
                multimodal = None
            ...
            if rid in finished:
                ...
                new.append((stream, rid, prompt, sp, multimodal))
...
class _AsyncLLMEngine:
    ...
    def add_request(self, seq_id: str, prompt: Union[str, List[int]], sampling_params: SamplingParams,
        ...
        visual_contract_keys = {
            'images', 'images_spatial_crop', 'images_seq_mask',
            'token_modalities', 'num_visual_tokens', 'multimodal_contract',
            'multimodal_embedding_protocol',
        }
        if multimodal and any(key in multimodal for key in visual_contract_keys):
            mask = multimodal.get('images_seq_mask')
            modalities = multimodal.get('token_modalities')
            if mask is None or modalities is None:
                raise MultimodalContractError(
                    "multimodal request requires mask and token modalities"
                )
            if len(mask) != len(prompt) or len(modalities) != len(prompt):
                raise MultimodalContractError(
                    "multimodal row metadata must align with prompt tokens"
                )
            if any(type(value) is not bool for value in mask):
                raise MultimodalContractError("images_seq_mask entries must be bool")
            seq.token_modalities = modalities
            seq.images_seq_mask = mask
            seq.num_visual_tokens = sum(mask)
            declared_visual = multimodal.get(
                'num_visual_tokens', seq.num_visual_tokens
            )
            if isinstance(declared_visual, bool) or not isinstance(
                declared_visual, int
            ):
                raise MultimodalContractError(
                    "declared visual token count must be an integer"
                )
            if declared_visual != seq.num_visual_tokens:
                raise MultimodalContractError("declared visual token count changed")
            contract = multimodal.get('multimodal_contract')
            if seq.num_visual_tokens and contract not in SUPPORTED_CONTRACTS:
                raise MultimodalContractError(
                    f"unsupported multimodal contract {contract!r}"
                )
            embedding_protocol = multimodal.get('multimodal_embedding_protocol')
            if (
                seq.num_visual_tokens
                and embedding_protocol not in SUPPORTED_EMBEDDING_PROTOCOLS
            ):
                raise MultimodalContractError(
                    f"unsupported multimodal embedding protocol "
                    f"{embedding_protocol!r}"
                )
            seq.multimodal_contract = contract or CONTRACT_VERSION
            seq.images = multimodal.get('images') if seq.num_visual_tokens else None
            seq.images_spatial_crop = (
                multimodal.get('images_spatial_crop')
                if seq.num_visual_tokens else None
            )
            if seq.num_visual_tokens and (
                seq.images is None or seq.images_spatial_crop is None
            ):
                raise MultimodalContractError(
                    "visual request is missing official images/spatial crop"
                )
            seq.multimodal_payload_encoded = False
        _apply_request_timing_metadata(seq, multimodal)
...
class AsyncLLMEngine:
    ...
    async def _engine_step(self) -> bool:
        ...
        for tup in new:
            ...
            if len(tup) == 5:
                stream, rid, prompt, sp, multimodal = tup
            else:
                stream, rid, prompt, sp = tup
                multimodal = None
            try:
                self._engine.add_request(rid, prompt, sp, multimodal=multimodal)
            except Exception as e:
                stream.put(e)
                stream.finish()
        for rid in finished:
    ...
    async def generate(
        ...
        if not self._loop_task or self._loop_task.done():
            self.start()
        stream = self._tracker.add_request(request_id, prompt, sampling_params, multimodal=multimodal)
        try:
